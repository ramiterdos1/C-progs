...
int calculate_bid(int player,int pos,int* first_moves,int* second_moves) {
    ...
    { 
      ...
      if(m==o_amt)
        {   if((player&1)==((draw+1)&1))val=(i>6)?min(o_lmv,p)+1:min(o_lmv,p)-1;
                                ...
                                else   val=(diff==0)?o_lmv+1:min(o_lmv,oppo)+i%2;//o_lmv chk can b used
                              }
            else val=1; 
           }
    ...
    /*if(p>oppo ){if(o_lmv<p)val=((p+oppo)/2+o_lmv)/2;
              else val=(p+oppo)/2;
             }
     else if(p<oppo)val=(p+o_lmv+oppo)/3+diff;
     else if((player&1 && (draw+1)&1)|| (player&1==0&&draw&1))val=o_lmv;
            else val=o_lmv+diff;
    if(f>1 && (m+1-val)<0)if(oppo<m)val=oppo-1;
                          else val=1;    
    if(o_pos<=2) val=(o_lmv+oppo)/2-2;
    if(f<=2){if(m>o_amt)val=(p+oppo)/2;
                   else val=2; 
            }     
    if((f>=7)&& (m-o_amt)>=30)val=(oppo+o_lmv)/2-2;
    if(f>1 && (m+1-val)<0)if(oppo<2*m)val=oppo-1;
                          else val=1;
    if(i==0)val=1;*/
    if(val<=0)val=1;
    if(m<=0)val=0;
